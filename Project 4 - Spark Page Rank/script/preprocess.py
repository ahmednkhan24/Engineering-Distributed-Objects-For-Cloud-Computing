...
# define all foreign characters found within the dblp.dtd
...
input_file_name = 'dblp.xml'

# location to write data to
output_file_name = 'dblp.csv'
...
with open(output_file_name, 'w') as csv_file:
    ...
    # loop through each inproceedings tag
    for node in tree.iter('inproceedings'):

        # array to hold each author involved in a single publication
        data = []

        # loop through each author per publication
        for author in node.iter('author'):

            # add the author to the array
            data.append(author.text)

        for booktitle in node.iter('booktitle'):

            # add the booktitle to the array, aka the venue name
            data.append(booktitle.text)

        # the publication is done and the authors and the venues have been saved, write to file
    ...
    for node in tree.iter('article'):
        ...
        for journal in node.iter('journal'):
            # add the journal to the array, aka the venue name
            data.append(journal.text)

        # the publication is done and the authors and the venues have been saved, write to file
        csv_writer.writerow(data)

    # loop through each proceedings tag
    for node in tree.iter('proceedings'):
        ...
        for booktitle in node.iter('title'):
            # add the booktitle to the array, aka the venue name
            data.append(booktitle.text)
